...
if os.path.exists('../../model/preTraining/CCLPLCAttackSurface_holdout.pickle'):
    with open('../../model/preTraining/CCLPLCAttackSurface_holdout.pickle','rb') as f:
        clf = pickle.load(f)
else:        
    df = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCLPLCAttackSurface_70.csv' )
    y=df['Attack Surface']
    clf = BaggingClassifier(base_estimator=SVC(),n_estimators=10, random_state=0).fit(df, y)
    with open('../../model/preTraining/CCLPLCAttackSurface_holdout.pickle','wb') as fw:
        pickle.dump(clf,fw)

df2 = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCLPLCAttackSurface_30.csv' )
predict_results=clf.predict(df2)
y2=df2['Attack Surface']
...
df3 = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCLPLCAttackSurface.csv')
y3=df3['Attack Surface']
if os.path.exists('../../model/preTraining/CCLPLCAttackSurface_10fold.pickle'):
    with open('../../model/preTraining/CCLPLCAttackSurface_10fold.pickle','rb') as f:
        clf3 = pickle.load(f)
else:         
    clf3 = BaggingClassifier(base_estimator=SVC(),n_estimators=10, random_state=0).fit(df3, y3)
    with open('../../model/preTraining/CCLPLCAttackSurface_10fold.pickle','wb') as fw:
        pickle.dump(clf3,fw)        
