...
if os.path.exists('../../model/preTraining/CCCExecutionTime_holdout.pickle'):
    with open('../../model/preTraining/CCCExecutionTime_holdout.pickle','rb') as f:
        clf = pickle.load(f)
else:      
    df = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCCExecutionTime_70.csv' )
    y=df['Execution Time']
    clf = BaggingClassifier(base_estimator=SVC(),n_estimators=10, random_state=0).fit(df, y)
    with open('../../model/preTraining/CCCExecutionTime_holdout.pickle','wb') as fw:
        pickle.dump(clf,fw)
		
df2 = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCCExecutionTime_30.csv' )
predict_results=clf.predict(df2)
y2=df2['Execution Time']
...
df3 = pd.read_csv('C:/Research/TOOL/DistFax_Meterial/data/ML/CCCExecutionTime.csv' )
y3=df3['Execution Time']
if os.path.exists('../../model/preTraining/CCCExecutionTime_10fold.pickle'):
    with open('../../model/preTraining/CCCExecutionTime_10fold.pickle','rb') as f:
        clf3 = pickle.load(f)
else:         
    clf3 = BaggingClassifier(base_estimator=SVC(),n_estimators=10, random_state=0).fit(df3, y3)
    with open('../../model/preTraining/CCCExecutionTime_10fold.pickle','wb') as fw:
        pickle.dump(clf3,fw)  
