...
import os
import pickle
if os.path.exists('../../model/preTraining/CCLPLCAttack_KMeans.pickle'):
    with open('../../model/preTraining/CCLPLCAttack_KMeanst.pickle','rb') as f:
        clf = pickle.load(f)
else:      
    X = pd.read_csv('../../data/ML/CCLPLC11817.csv')
    clf = KMeans(n_clusters=2)
    clf=clf.fit(X)
    with open('../../model/preTraining/CCLPLCAttack_KMeans.pickle','wb') as fw:
        pickle.dump(clf,fw)
...
CCLPLAttack=pd.read_csv('../../data/ML/CCLPLCAttack11817.csv')
...
CCLPLAttack5000=pd.read_csv('../../data/ML/CCLPLCAttack5065.csv')
